...
export class StandardResponse {
  ...
  private static resolveSuccessMessage(
    ...
    if (statusCode === 201) {
      return SYS_MSG.REQ_CYCLE.CREATED;
    }

    if (statusCode === 202) {
      return SYS_MSG.REQ_CYCLE.ACCEPTED;
    }

    if (statusCode === 204) {
      return SYS_MSG.REQ_CYCLE.NO_CONTENT;
    }

    switch (method?.toUpperCase()) {
      case 'GET':
        return SYS_MSG.REQ_CYCLE.RETRIEVED;
      case 'POST':
        return SYS_MSG.REQ_CYCLE.SUCCESS;
      case 'PATCH':
      case 'PUT':
        return SYS_MSG.REQ_CYCLE.UPDATED;
      case 'DELETE':
        return SYS_MSG.REQ_CYCLE.DELETED;
      default:
        return SYS_MSG.REQ_CYCLE.SUCCESS;
    }
  }
