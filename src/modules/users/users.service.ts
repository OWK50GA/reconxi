...
export class UsersService {
    ...
    private async validateAndEncryptCredentials(dto: CreateUserDto): Promise<{
        ...
        return {
            encryptedEmail, emailHash, passwordHash
        }
    }

    async create(dto: CreateUserDto): Promise<User> {
        const { firstName, lastName } = dto;
        const { encryptedEmail, passwordHash, emailHash } = await this.validateAndEncryptCredentials(dto);
    ...
    async createSession(userId: string, dto?: CreateSessionDto): Promise<Session> {
        const expiresAt = new Date(Date.now() + this.cryptoService.getExpiry("refresh"));
    ...
    async findSessionById(sessionId: string): Promise<Session> {
        ...
        if (!session) throw new UnauthorizedException(SYS_MSG.AUTH.INVALID_SESSION_ID);
